...
def process_doc(query, doc, match_obj, args):
    ...
    absolute_path = b64encode(
        doc.url.replace("file://", "", 1).encode("utf-8")
    ).decode("utf-8")

    colors = {
        "abstract": args.color_abstract,
        "file_path": args.color_path,
        "title": args.color_title,
        "file_info": args.color_file_info,
    }
    ...
    if args.show_abstract:
        if args.synthetic_abstract:
            res["abstract"] = (
                escape(query.makedocabstract(doc, methods=match_obj), False)
                .strip()
                .replace("\0", "")
            )
            ...
            res["abstract"] = (
                escape(doc.abstract, False).strip().replace("\0", "")
            )
            res["abstract"] = query.highlight(
                res["abstract"], ishtml=0, methods=match_obj
            )
...
def handle_args():
    ...
    parser.add_argument(
        "--theme-search",
        default=None,
        help="Theme used when issuing the query.",
    )
    parser.add_argument(
        "--theme-result",
        default=None,
        help="Theme used when displaying the results.",
    )
    parser.add_argument(
        "-p",
        "--strip-path",
        default=getenv("HOME", None),
    ...
    parser.add_argument(
        "--search-mesg", default=None, help="Message for the search prompt"
    )
    parser.add_argument(
        "--result-mesg", default=None, help="Message for the result prompt"
    )
    ...
    parser.set_defaults(
        show_abstract=True, xclip_copy=False, synthetic_abstract=False
    )
...
def main(argv):
    ...
    if not args.query_string:
        ...
        if args.theme_search:
            rofi_search_cmd += ["-theme", args.theme_search]
    ...
    recoll_db = recoll.connect(
        confdir=args.recoll_confdir, extra_dbs=args.extra_dbs
    )
    ...
    if args.theme_result:
        rofi_results_cmd += ["-theme", args.theme_result]

    print(rofi_results_cmd)

    # Pipe formatted results to Rofi
    rofi = Popen(rofi_results_cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE)
    selected_match = rofi.communicate(input=results_formatted.encode("utf-8"))[
        0
    ].decode("utf-8")
